...
def calcPosterior(priors: Tuple[float], conds: List[List[float]])->Tuple[Tuple[float]]:
	...
	# Loops over measures and multiples corresponding values
...
def normPosterior(posteriors: Tuple[Tuple[float]])->Tuple[Tuple[float]]:
	# Normalizes all posteriors value over summ of classes, providing real posterior
	M = len(posteriors)
	K = len(posteriors[0])
	normPosterior = []
	for d in range(M):
...
def calcClassConditionalProb(measures: Tuple[int], tags: Tuple[int], dimen: Tuple[int], K: int, delta: float=0.0)->List[List[float]]:
	...
	# of possible class values. Unseen values are updated by additive smoothing factor delta. 
	...
	# Preallocating to force memory error if one occurs. Laplace smooths 0 values.
...
def calcMeasProb(priors: Tuple[float], conds: List[List[float]])-> Tuple[float]:
	# Given priors and conditionals, calculates distribution of measures/data
	K = len(priors)
	M = len(conds[0])
	newProbs = []
...
class BayesClassifier:
	...
	# cond is K tables of class conditional probabilities. egain is economic gain matrix.
	# If no matrix is present, generates identity matrix. 
	def __init__(self, prior: Tuple[float], cond: List[List[float]], eGain: Tuple[Tuple[float]] = False):
		self.prior = prior
		self.cond = cond
		self.range = len(cond)
	...
	def assign(self, test: Tuple[int])-> Tuple[int]:
		# Assigns set of values to estimated class
		results = []
		K = self.range
		posterior = calcPosterior(self.prior, self.cond)
		for measure in test:
			# Precalculates posterior likliehoods. (Pd is ignored due to being shared normalizing factor
			...
			for assigned in range(1,K):
				# Calculates expected gain for each class.
...
## Tests calculation of measures
	...
	print("Posteriors are: {}. Should ideally be {}".format(posts[0], ".2, .24,.18")) # fist should be around .2, .24, .18
