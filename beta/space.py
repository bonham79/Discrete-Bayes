...
# being randomly generated between 2 and N or the default limit in config. 
	dimen = [randint(2, CARDINALITY_RANGE_LIMIT) for i in range(Z)]
	return tuple(dimen)
...
def genGain(K: int, identity=False)-> List[List[int]]: ####LINEAR ARRAY?
### Generates either a penalty matrix or identity matrix determined by 'identity'
...
### minimal and maximal. Diagonals will have max values throughout. 
### If passed optional identity boolean, will generate identity matrix.
	...
	for i in range(K):
		for j in range(K):
			# Always chooses a value less than correct identification for row.
			eGain[i][j] = minimal
